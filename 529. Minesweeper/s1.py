...
#************************************************************	
    def countMine(self, board, click):
		...
		count = 0
		if (row-1) > 0:
			if (col-1>0): if board[row-1][col-1] == 'M': count+=1
			if board[row-1][col] == 'M': count+=1
			if (col+1<max_col): if board[row-1][col+1] == 'M': count+=1

		if (col-1>0): if board[row][col-1] == 'M': count+=1
		if (col+1<max_col): if board[row][col+1] == 'M': count+=1
